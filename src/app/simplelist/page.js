...
export default function SimpleList() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data.theme) {
        setThemeState({
          moodName: data.theme.moodName || mood,
  ...
  return (
    <MoodBackground
      mood={themeState.moodName}
      hexColor={themeState.hexColor}
    >
      <div className="min-h-screen flex flex-col items-center p-6 font-sans text-white">
        ...
        {themeState.moodName !== "neutral" && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="mb-4 text-xs font-bold tracking-widest uppercase text-white/60 bg-white/5 px-4 py-1 rounded-full"
          >
            {themeState.moodName}
          </motion.div>
        )}
          ...
          <div className="relative z-10 flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1">
                <label className="text-xs text-white/50 font-bold uppercase tracking-widest ml-1 mb-2 block">
                  Genre (Optional)
                </label>
                <input
              ...
              <button
                ...
                className="self-end w-full sm:w-auto bg-white text-black hover:bg-gray-200 font-bold h-[54px] px-8 rounded-xl transition-all transform active:scale-95 disabled:opacity-50 flex justify-center items-center gap-2 shadow-lg"
              >
                {loading ? (
        ...
        <div className="w-full max-w-2xl space-y-4 pb-12">
            ...
            {recommendations.map((song, i) => (
              ...
              >
                <div className="flex-1 pr-4">
                  ...
                  <p className="text-white/60 text-sm font-medium mb-2 ml-11">
                    {song.artist}
                  </p>
                  <p className="text-white/30 text-xs italic ml-11 border-l-2 border-white/10 pl-3">
                    "{song.reason}"
                  </p>
                </div>

                {/* ✅ LINK FIX:
                   We build the YouTube URL right here using the title and artist.
                   This works even if the API doesn't send a 'link' property.
                */}
                <a
          ...
          {!loading && recommendations.length === 0 && mood && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-center text-white/20 mt-12 text-sm"
            >
